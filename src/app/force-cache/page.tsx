...
export default async function Page() {
  ...
  const local2 = await (await fetch('http://localhost:4000/api', { next: { tags: ['api'] } })).json()
  ...
  return (
    <>
      ...
      <div>
        Local (revalidate: 10): {local2.value}<br />
      ...
      <div>
        External API (randomnumberapi.com): {external2[0]}<br />
      </div>
      <div>
